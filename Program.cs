...
/* 1. Criando Objetos (Instanciando a classe PessoaNova)
            PessoaNova PessoaNova1 = new PessoaNova("Alice", 30);
           PessoaNova PessoaNova2 = new PessoaNova("Bob", 25);

           // 2. Usando Métodos de Instância (Métodos não estáticos)
           PessoaNova1.ExibirInformacoes();
           PessoaNova2.ExibirInformacoes();

           // 3. Usando Método Estático (Pertence à Classe, não à instância)
           PessoaNova.ExibirNumeroDePessoaNovas();

           // 4. Membros de Instância vs. Membros Estáticos
           Console.WriteLine($"PessoaNova1: {PessoaNova1.Nome}, Idade: {PessoaNova1.Idade}");
           Console.WriteLine($"PessoaNova2: {PessoaNova2.Nome}, Idade: {PessoaNova2.Idade}");
           Console.WriteLine($"Número total de PessoaNovas: {PessoaNova.NumeroDePessoaNovas}"); */
...
/*  class Program
 {
     ...
     {
         Livro livro = new Livro("O Senhor dos Anéis", "J.R.R. Tolkien", 1954, 432);
...
(string), modelo (string) e ano (int). Crie um objeto dessa classe e inicialize suas propriedades. */


/* Carro Mycar = new Carro("Toyota", "Corolla", 2009);
Console.WriteLine($"Marca: {Mycar.Marca}, Modelo: {Mycar.Modelo}, Ano: {Mycar.Ano}"); */

/* 4. Defina uma classe chamada PessoaNova com as propriedades nome (string), idade (int) e
    cidade (string). Crie um objeto dessa classe e inicialize suas propriedades. 



// Criando um objeto da classe PessoaNova
PessoaNova pessoa = new PessoaNova
{
    Nome = "João",
    Idade = 30,
    Cidade = "São Paulo"
